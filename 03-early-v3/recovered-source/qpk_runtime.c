                              ...
                              int argc, JSValueConst *argv)
{
  ...
  if (g_qpk.widget_types[handle - 1] == QPK_WIDGET_BUTTON)
    {
      lv_label_set_text(lv_obj_get_child(g_qpk.widgets[handle - 1], 0), text);
    }
                           ...
                           int argc, JSValueConst *argv)
{
  ...
  lv_obj_set_style_radius(panel, qpk_arg_int(context, argc, argv, 5, 0), 0);
                               ...
                               int argc, JSValueConst *argv)
{
  ...
  qpk_input_button(box, "取消", root_width - 208, 0x40515a,
                   qpk_input_cancel);
...
static int qpk_storage_path(const char *key, char *path, size_t path_len)
{
  ...
  if (!qpk_storage_component_valid(key, QPK_STORAGE_KEY_MAX) ||
      !qpk_storage_component_valid(g_qpk.package, 48))
    {
      return -EINVAL;
    }
...
static int qpk_storage_prepare(void)
{
  ...
  ret = mkdir(QPK_DIR, 0777);
  if (ret < 0 && errno != EEXIST)
    {
      return -errno;
    }

  ret = mkdir(QPK_STORAGE_ROOT, 0777);
  return ret < 0 && errno != EEXIST ? -errno : 0;
}
