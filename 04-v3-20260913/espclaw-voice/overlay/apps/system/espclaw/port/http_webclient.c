...
static int send_body(void *buffer, size_t *size, const void **data,
                      size_t remaining, void *arg)
{
    (void)buffer;
    struct upload_buffer *upload = arg;
    int ret = request_state(upload->response);
    if (ret) return ret;
    if (upload->offset > upload->size || remaining != upload->size - upload->offset)
        return -EINVAL;
    size_t n = remaining > 8192 ? 8192 : remaining;
    *data = upload->data + upload->offset;
    *size = n;
    upload->offset += n;
    return 0;
}
...
static esp_err_t post_body(const claw_llm_http_json_request_t *req,
                          size_t payload_size, size_t limit, const char *content_type,
                          claw_llm_http_response_t *out, char **error)
{
    struct webclient_context *client = NULL;
    struct response_buffer body = {0};
    struct upload_buffer upload = {0};
    ...
    if (strncmp(req->url, "https://", 8) || !clean_value(req->url) ||
        ...
        payload_size > limit || req->header_count > HEADER_LIMIT ||
    ...
    headers[count++] = make_header("Content-Type", content_type);
    ...
    upload.data = (const unsigned char *)req->body;
    upload.size = payload_size;
    upload.response = &body;
    client->body_callback = send_body;
    client->body_callback_arg = &upload;
    client->bodylen = payload_size;
...
esp_err_t claw_llm_http_post_json(const claw_llm_http_json_request_t *req,
                                 claw_llm_http_response_t *out, char **error)
{
    size_t size = req && req->body ? strnlen(req->body, REQUEST_LIMIT + 1) : 0;
    return post_body(req, size, REQUEST_LIMIT, "application/json", out, error);
}
                               ...
                               size_t size, atomic_bool *cancel, char **response)
{
    ...
    *response = NULL;
    claw_llm_http_json_request_t req = {
        .url = url, .api_key = key, .auth_type = "bearer", .body = data,
        .timeout_ms = 30000, .abort_flag = cancel
    };
    claw_llm_http_response_t out = {0};
    char *error = NULL;
    int ret = post_body(&req, size, 1024 * 1024, content_type, &out, &error);
    free(error);
    if (ret == ESP_OK) { *response = out.body; return 0; }
    return ret == ESP_ERR_TIMEOUT ? -ETIMEDOUT :
           ret == ESP_ERR_INVALID_STATE ? -ECANCELED :
           ret == ESP_ERR_NOT_SUPPORTED ? -ENOTSUP : -EIO;
}
