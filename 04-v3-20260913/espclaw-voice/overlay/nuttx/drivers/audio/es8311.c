                               ...
                               int result)
{
  ...
  if (result < 0 && priv->audio_mode == ES_MODULE_ADC)
    {
      apb->nbytes = 0; /* Never expose timed-out DMA bytes as valid audio. */
    }
  priv->result = result;
...
#endif /* !CONFIG_AUDIO_MULTI_SESSION */
{
  ...
  /* The Espressif I2S receive worker marks a buffer FINAL unless streaming
   * has been enabled. Without this, the first microphone buffer ends RX. */
  ret = I2S_IOCTL(priv->i2s, AUDIOIOC_START, 0);
  if (ret < 0 && ret != -ENOTTY)
    {
