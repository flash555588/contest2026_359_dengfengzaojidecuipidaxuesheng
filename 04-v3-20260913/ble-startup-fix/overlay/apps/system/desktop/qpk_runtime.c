...
static void qpk_camera_bar_blit(uint16_t *pixels)
{
  int status;
  pthread_mutex_lock(&g_camera_lock);
  ...
  for (int y = 500; y < 600; y++)
    for (int x = 0; x < 1024; x++)
      {
        uint16_t color = 0x1082;
        int dx = x - 512;
        int dy = y - 550;
        int radius = dx * dx + dy * dy;
        if (radius < 38 * 38)
          color = status == 1 ? 0xffe0 : status < 0 ? 0xf800 :
                  status == 2 ? 0x07e0 : 0xffff;
        if (radius < 30 * 30 && radius > 27 * 27) color = 0x1082;
        /* Opposed arrows return to the camera selection controls. */
        if ((x >= 42 && x <= 108 && abs(y - 536) < 3) ||
            (x >= 42 && x <= 108 && abs(y - 564) < 3) ||
            (x >= 94 && x <= 108 && abs(abs(y - 536) - (108 - x)) < 3) ||
            (x >= 42 && x <= 56 && abs(abs(y - 564) - (x - 42)) < 3))
          color = 0xffff;
        if (x >= 930 && x < 990 && y >= 520 && y < 580 &&
            (abs((x - 960) - (y - 550)) < 3 ||
             abs((x - 960) + (y - 550)) < 3)) color = 0xffff;
  ...
  for (i = 0; i < QPK_MAX_WIDGETS; i++)
    {
      if (g_qpk.widgets[i] == NULL)
        {
          if (lv_obj_add_event_cb(object, qpk_widget_deleted, LV_EVENT_DELETE,
...
static void qpk_timer_cb(lv_timer_t *timer)
{
  ...
  int id = binding->id;
  result = qpk_call(binding->function, QPK_EVENT_BUDGET);
  /* The callback may cancel itself and reuse this slot for a new timer. */
  if (JS_IsException(result) && binding->used && binding->timer == timer &&
      binding->id == id)
    {
...
{
  if (g_photo_status == 1) g_photo_status = -ECANCELED;
  g_photo_pending = false;
  ...
  if (g_qpk.context != NULL)
    {
      ...
      for (i = 0; i < QPK_MAX_EVENTS; i++)
        {
          if (g_qpk.events[i].used && g_qpk.events[i].owner != NULL)
            {
              lv_obj_remove_event_cb(g_qpk.events[i].owner, qpk_event_clicked);
              lv_obj_remove_event_cb(g_qpk.events[i].owner, qpk_event_deleted);
            }
        }
      ...
      for (i = 0; i < QPK_MAX_TIMERS; i++)
        {
          if (g_qpk.timers[i].used)
            {
              qpk_timer_release(g_qpk.context, &g_qpk.timers[i]);
            }
        }
      ...
      for (i = 0; i < QPK_MAX_EVENTS; i++)
        {
          if (g_qpk.events[i].used)
            {
              qpk_event_release(g_qpk.context, &g_qpk.events[i]);
            }
        }
