...
with port, args.output.with_suffix('.log').open('xb') as log:
    for case in [{'name': 'console', 'command': ''}, *suite[args.start:]]:
        ...
        while time.monotonic() < deadline:
            ...
            if prompt in data and (data.endswith(b'\x1b[K') or data.endswith(prompt) or
                                   time.monotonic() - received_at > 0.3):
                finished = True
                break
        ...
        # Keep late asynchronous diagnostics associated with this command.
        until = time.monotonic() + min(30, case.get('settle', 0))
        while time.monotonic() < until:
            chunk = port.read(max(1, port.in_waiting))
            if chunk:
                log.write(chunk)
                log.flush()
                result['output'] += chunk.decode('utf-8', errors='replace').replace('\r', '')
        args.output.write_text(json.dumps(results, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
