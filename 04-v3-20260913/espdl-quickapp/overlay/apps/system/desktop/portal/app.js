...
for(const id of ['ai','homeassistant','music','voice','desktop']){
  $(id).onsubmit=e=>{e.preventDefault();run(async()=>{const form=e.target,button=form.querySelector('button');button.disabled=true;try{const data=Object.fromEntries(new FormData(form));for(const key of ['api_key','token','secret_id','secret_key'])if(data[key]==='')delete data[key];for(const key of ['max_tokens','timeout_ms','light','palette','widget','reduced_motion'])if(key in data)data[key]=Number(data[key]);await api('config?section='+id,'POST',data);form.querySelectorAll('input[type=password]').forEach(x=>x.value='');say(id==='desktop'?'外观已提交，设备正在应用和保存。':id==='voice'?'语音输入配置已保存。':'已保存，下次使用时生效。');if(id!=='desktop')await loadConfig(id);}finally{button.disabled=false;}});};
}
