...
static int spiflash_direct_read(uint32_t address, void *buffer,
                      uint32_t length)
{
  int ret = OK;
...
static int spiflash_direct_write(uint32_t address, const void *buffer,
                       uint32_t length)
{
  int ret = OK;
#if CONFIG_ESPRESSIF_SECURE_FLASH_ENC_ENABLED
...
static int spiflash_direct_erase(uint32_t address, uint32_t length)
{
  int ret = OK;
