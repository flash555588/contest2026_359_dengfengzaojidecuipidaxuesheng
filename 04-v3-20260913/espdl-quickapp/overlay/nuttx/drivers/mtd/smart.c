  ...
  for (sector = 0; sector < totalsectors; sector++)
    {
      ...
      /* Test if this sector has been committed.  An interrupted allocation
       * can leave the logical/sequence fields programmed while the commit
       * bit is still in its erased state.  Such a sector is not reusable:
       * flash programming cannot turn its zero bits back into ones.  The
       * old scan treated every uncommitted sector as free, although the
       * allocator only accepts a completely blank header.  That made the
       * in-RAM free count larger than the number of allocatable sectors and
       * eventually produced a false ENOSPC from smart_findfreephyssector().
       *
       * Account a non-blank, uncommitted sector as released.  Garbage
       * collection can then reclaim its erase block without exposing the
       * incomplete sector as live file-system data.
       */
      ...
      if ((header.status & SMART_STATUS_COMMITTED) ==
        ...
        {
#if SMART_STATUS_VERSION == 1
          if (logicalsector == UINT16_MAX &&
              *((FAR uint16_t *)&header.seq) == UINT16_MAX)
#else
          if (logicalsector == UINT16_MAX &&
...
/****************************************************************************
 * Local diagnostic patch: compare the in-RAM free count of every erase block
 * with what the media really contains, then dump the sector headers of the
 * block the allocator picked.  Called only when allocation fails.
 ****************************************************************************/

static void smart_qpk_diag(FAR struct smart_struct_s *dev, uint16_t allocblock)
{
  struct smart_sect_header_s hdr;
  uint32_t addr;
  uint32_t blank = 0;
  uint32_t dirty = 0;
  uint32_t released = 0;
  ...
  ferr("QPKDIAG: freesectors=%d releasesectors=%d allocblock=%d allocfree=%d\n",
       dev->freesectors, dev->releasesectors, allocblock,
       allocblock == 0xffff ? 0 : dev->freecount[allocblock]);

  if (allocblock < dev->neraseblocks)
    {
      for (s = allocblock * dev->sectorsperblk;
           s < allocblock * dev->sectorsperblk + dev->availsectperblk; s++)
        {
          addr = (uint32_t)s * dev->mtdblkspersector * dev->geo.blocksize;
          ret = MTD_READ(dev->mtd, addr, sizeof(hdr), (FAR uint8_t *)&hdr);
          if (ret != sizeof(hdr))
            {
              ferr("QPKDIAG: phys %d read error %d\n", s, ret);
              continue;
            }

          ferr("QPKDIAG: phys %d status=%02x seq16=%04x log=%04x\n",
               s, hdr.status, *((FAR uint16_t *)&hdr.seq),
               *((FAR uint16_t *)hdr.logicalsector));
        }
    }

  for (b = 0; b < dev->neraseblocks; b++)
    {
      uint16_t nblank = 0;

      for (s = b * dev->sectorsperblk;
           s < b * dev->sectorsperblk + dev->availsectperblk; s++)
        {
          addr = (uint32_t)s * dev->mtdblkspersector * dev->geo.blocksize;
          ret = MTD_READ(dev->mtd, addr, sizeof(hdr), (FAR uint8_t *)&hdr);
          if (ret != sizeof(hdr))
          ...
          if (*((FAR uint16_t *)hdr.logicalsector) == 0xffff &&
              *((FAR uint16_t *)&hdr.seq) == 0xffff &&
              (hdr.status & SMART_STATUS_COMMITTED) ==
               (CONFIG_SMARTFS_ERASEDSTATE & SMART_STATUS_COMMITTED))
            {
              blank++;
              nblank++;
            }
          else if ((hdr.status & SMART_STATUS_COMMITTED) ==
                   (CONFIG_SMARTFS_ERASEDSTATE & SMART_STATUS_COMMITTED))
            {
              dirty++;
            }
          else if ((hdr.status & SMART_STATUS_RELEASED) !=
                   (CONFIG_SMARTFS_ERASEDSTATE & SMART_STATUS_RELEASED))
            {
              released++;
            }
          else if ((hdr.status & SMART_STATUS_VERBITS) != SMART_STATUS_VERSION)
            {
              other++;
            }
          else
      ...
      if (nblank != dev->freecount[b])
        {
          if (shown < 24)
            {
              ferr("QPKDIAG: block %d freecount=%d blank=%d\n",
                   b, dev->freecount[b], nblank);
              shown++;
            }
  ...
  ferr("QPKDIAG: sectors=%d blank=%d dirty=%d released=%d live=%d other=%d "
       "blocks=%d mismatched=%d\n",
       dev->totalsectors, blank, dirty, released, live, other,
       dev->neraseblocks, mismatches);
}
