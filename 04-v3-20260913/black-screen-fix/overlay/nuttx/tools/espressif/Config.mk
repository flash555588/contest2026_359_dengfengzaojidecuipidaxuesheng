...
define MKIMAGE
	...
	$(eval ELF2IMAGE_OPTS := $(if $(CONFIG_ESPRESSIF_SIMPLE_BOOT),--ram-only-header) $(if $(CONFIG_ARCH_CHIP_ESP32P4),--min-rev-full $(CONFIG_ESP32P4_REV_MIN_FULL) --max-rev-full $(CONFIG_ESP32P4_REV_MAX_FULL)) -fs $(FLASH_SIZE) -fm $(FLASH_MODE) -ff $(FLASH_FREQ))
	esptool.py -c $(CHIP_SERIES) elf2image $(ELF2IMAGE_OPTS) -o nuttx.bin nuttx
	$(if $(and $(CONFIG_ESPRESSIF_SIMPLE_BOOT),$(CONFIG_ARCH_CHIP_ESP32P4)),python3 tools/espressif/simple_boot_digest.py nuttx.bin)
	$(Q) echo nuttx.bin >> nuttx.manifest
