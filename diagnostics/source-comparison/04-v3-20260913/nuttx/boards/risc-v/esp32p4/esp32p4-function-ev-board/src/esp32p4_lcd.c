...
#include "esp32p4-function-ev-board.h"
    ...
    defined(CONFIG_ESPRESSIF_MIPI_CSI)
#  define LCD_PSRAM_FRAMEBUFFER 1
#endif

#ifdef LCD_PSRAM_FRAMEBUFFER
...
#if defined(LCD_PSRAM_FRAMEBUFFER) && \
...
int up_fbinitialize(int display)
{
   ...
   * one DMA scanout frame; direct mode keeps three contiguous pages for
   * VSync page flipping.
   */
...
#elif defined(LCD_PSRAM_FRAMEBUFFER)
  if (!esp_psram_is_initialized())
    {
      syslog(LOG_ERR, "ERROR: PSRAM user heap is unavailable\n");
      return -ENOMEM;
