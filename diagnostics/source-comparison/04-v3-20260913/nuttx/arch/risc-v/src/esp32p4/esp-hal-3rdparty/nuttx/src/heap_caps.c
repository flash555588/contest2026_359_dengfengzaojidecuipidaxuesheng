...
static inline bool esp_heap_caps_use_user_heap(uint32_t caps)
{
  /* Large SPIRAM/DMA frame buffers must not consume the tiny kernel heap. */

  return (caps & (MALLOC_CAP_SPIRAM | MALLOC_CAP_DMA)) != 0 &&
         (caps & MALLOC_CAP_INTERNAL) == 0;
}
#else
#  define esp_heap_caps_use_user_heap(caps) false
#endif
...
void *heap_caps_malloc(size_t size, uint32_t caps)
{
  ...
  if (esp_heap_caps_retention_caps_exact(caps))
    ...
    {
      if (esp_heap_caps_use_user_heap(caps))
        {
          ptr = kumm_malloc(size);
        }
      else
        {
          ptr = kmm_malloc(size);
...
void *heap_caps_aligned_alloc(size_t alignment, size_t size, uint32_t caps)
{
  ...
  if (esp_heap_caps_retention_caps_exact(caps))
  ...
  else
    {
      if (esp_heap_caps_use_user_heap(caps))
        {
          ptr = kumm_memalign(alignment, size);
        }
      else
        {
                               ...
                               uint32_t caps)
{
  ...
  if (esp_heap_caps_retention_caps_exact(caps))
  ...
  else
    {
      if (esp_heap_caps_use_user_heap(caps))
        {
          ptr = kumm_memalign(alignment, total);
        }
      else
        {
