...
#  define V1_BOOT_TRACE(s) ets_printf(s)
#else
#  define V1_BOOT_TRACE(s) ((void)0)
#endif
/****************************************************************************
...
static void hardware_initialize(void)
{
  ...
  V1_BOOT_TRACE("V1: arch initialize begin\n");
  up_initialize();
  V1_BOOT_TRACE("V1: arch initialize end\n");

  /* Initialize common drivers */

  V1_BOOT_TRACE("V1: drivers initialize begin\n");
  drivers_initialize();
  V1_BOOT_TRACE("V1: drivers initialize end\n");
  ...
  boards_trace_begin();
  V1_BOOT_TRACE("V1: board early begin\n");
  board_early_initialize();
  V1_BOOT_TRACE("V1: board early end\n");
  boards_trace_end();
#endif
...
void nx_start(void)
{
  ...
  V1_BOOT_TRACE("V1: tasklist begin\n");
  tasklist_initialize();
  V1_BOOT_TRACE("V1: tasklist end\n");

  /* The memory manager has been initialized */

  V1_BOOT_TRACE("V1: memory begin\n");
  memory_initialize();
  V1_BOOT_TRACE("V1: memory end\n");

  /* MCU-specific hardware is initialized */

  V1_BOOT_TRACE("V1: hardware begin\n");
  hardware_initialize();
  V1_BOOT_TRACE("V1: hardware end\n");

  /* Setup for Multi-Tasking ************************************************/

  /* Announce that the CPU0 IDLE task has started */

  V1_BOOT_TRACE("V1: idle note begin\n");
  sched_note_start(&this_cpu_var(g_idletcb));
  V1_BOOT_TRACE("V1: idle note end\n");
  ...
  V1_BOOT_TRACE("V1: idle files begin\n");
  DEBUGVERIFY(group_setupidlefiles());
  V1_BOOT_TRACE("V1: idle files end\n");
  ...
  V1_BOOT_TRACE("V1: smp start begin\n");
  DEBUGVERIFY(nx_smp_start());
  V1_BOOT_TRACE("V1: smp start end\n");
  ...
  V1_BOOT_TRACE("V1: bringup begin\n");
  nx_bringup();
  V1_BOOT_TRACE("V1: bringup end\n");
