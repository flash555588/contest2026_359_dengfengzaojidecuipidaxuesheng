...
class MyInt8Quantizer(BaseQuantizer):
    def __init__(
        ...
        if sym:
            ...
            self.policy = QuantizationPolicy(
                QuantizationProperty.PER_TENSOR + QuantizationProperty.LINEAR + QuantizationProperty.ASYMMETRICAL
            )
    ...
    def quant_operation_types(self) -> set:
        return {'Conv', 'ConvTranspose', 'MatMul', 'Gemm', 'Relu', 'Clip', 'Sub', 'Abs', 'Mul', 'LayerNormalization'}
...
class MyOptimPass(QuantizationOptimizationPass):
    ...
    def optimize(self, graph: BaseGraph, **kwargs) -> None:
        ...
        patterns = processor.pattern_matching(patterns=['Relu', 'Clip'], edges=[[0, 1]], exclusive=True)
        ...
        for relu, clip in patterns:
            ...
            if not isinstance(relu, QuantableOperation):
                continue
            relu.config.output_quantization_config[0].dominated_by = clip.config.output_quantization_config[0]
            clip.config.input_quantization_config[0].dominated_by = clip.config.output_quantization_config[0]
...
with ENABLE_CUDA_KERNEL():
    ...
    dispatching = PFL.Dispatcher(graph=graph).dispatch(quant_types=quantizer.quant_operation_types)
    ...
    graphwise_error_analyse(
        graph=graph, running_device='cuda', dataloader=calibration_dataloader, collate_fn=lambda x: x.cuda()
    )
