...
needed = ['qpk_storage_read', 'qpk_storage_write', 'qpk_storage_remove', 'qpk_storage_selftest',
...
report = {'status': 'static-checks-passed', 'firmware_sha256': hashlib.sha256(image).hexdigest(),
          ...
          'config_identical_to_wifi_fix': True, 'prior_fix_overlay_unchanged': True,
...
for path in sorted((delivery / 'overlay/apps/system/desktop').iterdir()):
    ...
    previous = workspace / 'diagnostics/storage-comparison' / ('current-' + path.name)
...
(delivery / 'storage.patch').write_text(''.join(patch), encoding='utf-8', newline='\n')
