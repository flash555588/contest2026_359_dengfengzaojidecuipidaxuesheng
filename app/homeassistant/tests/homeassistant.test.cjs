...
test('c client stays in the contest app tree', () => {
  ...
  assert.match(src, /return -EACCES/);
  assert.match(src, /return -EOPNOTSUPP/);
  assert.doesNotMatch(src, /strncmp\(url, "http:\/\/", 7\) != 0/);
  const app = fs.readFileSync(path.join(qa, 'app.js'), 'utf8');
  assert.match(app, /https:\/\/homeassistant\.local:8123/);
});
