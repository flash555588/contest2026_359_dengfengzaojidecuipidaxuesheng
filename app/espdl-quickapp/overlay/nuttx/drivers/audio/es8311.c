...
static void es8311_trace_state(FAR struct es8311_dev_s *priv,
...
/****************************************************************************
 * Name: es8311_trace_state
 *
 * Description:
 *   Temporary switch-path diagnostics.  These registers distinguish a muted
 *   DAC from an I2S stream that has stopped producing clock/data.
 *
 ****************************************************************************/
...
static void es8311_trace_state(FAR struct es8311_dev_s *priv,
                               FAR const char *phase)
{
  printf("[audio-state] codec %s mode=%d rate=%" PRIu32
         " 09=%02x 0d=%02x 0e=%02x 12=%02x 14=%02x"
         " 31=%02x 32=%02x 37=%02x 44=%02x 45=%02x\n",
         phase, priv->audio_mode, priv->samprate,
         es8311_readreg(priv, ES8311_SDPIN_REG09),
         es8311_readreg(priv, ES8311_SYSTEM_REG0D),
         es8311_readreg(priv, ES8311_SYSTEM_REG0E),
         es8311_readreg(priv, ES8311_SYSTEM_REG12),
         es8311_readreg(priv, ES8311_SYSTEM_REG14),
         es8311_readreg(priv, ES8311_DAC_REG31),
         es8311_readreg(priv, ES8311_DAC_REG32),
         es8311_readreg(priv, ES8311_DAC_REG37),
         es8311_readreg(priv, ES8311_GPIO_REG44),
         es8311_readreg(priv, ES8311_GP_REG45));
}
                               ...
                               int result)
{
  ...
  /* Preserve the first stream error until it reaches the upper half. */

  if (result < 0 && priv->result == OK)
    {
      priv->result = result;
    }
...
static int es8311_processbegin(FAR struct es8311_dev_s *priv)
{
  ...
  while (priv->inflight < CONFIG_ES8311_INFLIGHT &&
         dq_peek(&priv->pendq) != NULL && !priv->paused && !priv->terminating)
    {
      ...
      if (ret < 0)
        {
          ...
          /* A rejected submission will never receive an I2S callback. */
...
#endif /* !CONFIG_AUDIO_MULTI_SESSION */
{
  ...
  if (priv->audio_mode == ES_MODULE_DAC ||
      priv->audio_mode == ES_MODULE_ADC_DAC)
    {
      ...
      /* STOP sets DAC_REG31[6:5] to mute the analog DAC.  A software reset
       * does not reliably restore that register while the codec clocks are
       * stopped, so every new playback stream must explicitly clear it. */

      dac_mute = es8311_readreg(priv, ES8311_DAC_REG31) & 0x9f;
      ret |= es8311_writereg(priv, ES8311_DAC_REG31, dac_mute);
    }
  ...
  /* STOP/PAUSE state belongs to the previous stream.  A new reservation
   * normally clears it, but resetting it here as well makes a completed
   * capture followed immediately by playback deterministic and prevents
   * processbegin() from silently leaving every new buffer pending.
   */

  priv->paused = false;
  priv->terminating = false;
  priv->result = OK;
  /* ESP I2S needs the stream lifetime, otherwise RX marks every buffer FINAL. */
...
#endif /* !CONFIG_AUDIO_MULTI_SESSION */
{
  ...
  /* Block new submissions before aborting in-flight DMA. A codec with no
   * incoming clock cannot produce the EOF that the old join path awaited.
   */
  ...
  priv->paused = true;
  nxmutex_unlock(&priv->pendlock);
  ret = I2S_IOCTL(priv->i2s, AUDIOIOC_STOP, 0);
  if (ret < 0)
    {
      return ret;
    }
  ...
  switch (cmd)
    {
      ...
      /* The audio upper half requires this limit even with generic buffers. */
...
#endif /* !CONFIG_AUDIO_MULTI_SESSION */
{
  ...
  /* START can fail or a client can close a prepared stream before it was
   * started.  Such buffers never reach the I2S completion callback, so drop
   * the codec's references here before making the session reservable again.
   */
...
static void *es8311_workerthread(pthread_addr_t pvarg)
{
  ...
  /* All DMA has returned. Stop shared I2S clocks before changing its format. */
  I2S_IOCTL(priv->i2s, AUDIOIOC_STOP, 0);
