...
#ifdef CONFIG_ESP32P4_BOARD_MICROPHONE
...
#if defined(CONFIG_ESPRESSIF_SPI3) && defined(CONFIG_MMCSD_SPI)
...
#if defined(CONFIG_ESPRESSIF_SPI3) && defined(CONFIG_MMCSD_SPI)
/* LDO_VO4 supplies the SD1 signal pull-ups and IO voltage rail.  Keep the
 * channel acquired for as long as the socket is in use. */

static struct esp_ldo_config_t g_sdcard_ldo =
{
  .chan_id = 4,
  .voltage_mv = 3300,
  .handler = NULL,
...
int esp_bringup(void)
{
...
#if defined(CONFIG_ESPRESSIF_SPI3) && defined(CONFIG_MMCSD_SPI)
  /* SD1_D0..D3 are GPIO39..42 and SD1_CLK/CMD are GPIO43/44. The socket has
   * no CD/WP signals, so an absent or unformatted card must not block boot. */
  ret = esp_ldo_channel_acquire(&g_sdcard_ldo);
  if (ret < 0)
    {
      syslog(LOG_ERR, "ERROR: failed to enable SD IO LDO_VO4: %d\n", ret);
    }
  else
    {
      syslog(LOG_INFO, "INFO: SD IO LDO_VO4 enabled at 3.3V\n");
      usleep(10000);
    }
  ...
  if (ret < 0 && errno != EEXIST)
    {
      syslog(LOG_ERR, "ERROR: failed to create /sdcard: %d\n", errno);
    }
...
#  ifdef CONFIG_FS_FAT
    ...
    {
      ret = nx_mount("/dev/mmcsd0", "/sdcard", "vfat", 0, NULL);
      if (ret < 0)
        {
          /* Enumeration can finish just after the block device is bound. */
          usleep(100000);
          ret = nx_mount("/dev/mmcsd0", "/sdcard", "vfat", 0, NULL);
        }
      ...
      if (ret < 0)
        {
          syslog(LOG_ERR, "ERROR: failed to mount /dev/mmcsd0 at /sdcard: %d\n",
                 ret);
        }
      else
        {
          syslog(LOG_INFO, "INFO: SD card mounted at /sdcard\n");
        }
    }
#  endif
#endif
...
#  elif defined(CONFIG_ESPRESSIF_SPI2)
  /* Quick apps claim shared extension pins before starting SPI2. Initializing
   * it here would seize those pins before the GPIO/UART owner is known. */
#  endif
...
#  ifdef CONFIG_ESP32P4_SELECTS_REV_LESS_V3
  syslog(LOG_ALERT, "V1 bringup: spiflash begin\n");
#  endif
  ret = board_spiflash_init();
#  ifdef CONFIG_ESP32P4_SELECTS_REV_LESS_V3
  syslog(LOG_ALERT, "V1 bringup: spiflash end %d\n", ret);
#  endif
  if (ret)
    {
...
#  ifdef CONFIG_ESP32P4_SELECTS_REV_LESS_V3
  syslog(LOG_ALERT, "V1 bringup: camera begin\n");
#  endif
  ret = board_camera_initialize();
#  ifdef CONFIG_ESP32P4_SELECTS_REV_LESS_V3
  syslog(LOG_ALERT, "V1 bringup: camera end %d\n", ret);
#  endif
  if (ret < 0)
    {
