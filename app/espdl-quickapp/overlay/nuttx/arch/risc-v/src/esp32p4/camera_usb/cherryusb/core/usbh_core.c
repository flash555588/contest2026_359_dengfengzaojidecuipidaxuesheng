...
static int parse_config_descriptor(struct usbh_hubport *hport, struct usb_configuration_descriptor *desc, uint16_t length)
{
    ...
    if (desc->bNumInterfaces > CONFIG_USBHOST_MAX_INTERFACES) return -USB_ERR_INVAL;
    if (desc->bLength != USB_SIZEOF_CONFIG_DESC) {
        ...
        while (desc_len < length) {
            if (length - desc_len < 2 || p[0] < 2 || p[0] > length - desc_len) return -USB_ERR_INVAL;
            switch (p[DESC_bDescriptorType]) {
                ...
                case USB_DESCRIPTOR_TYPE_ENDPOINT:
                    ...
                    if (cur_iface >= CONFIG_USBHOST_MAX_INTERFACES || cur_alt_setting >= CONFIG_USBHOST_MAX_INTF_ALTSETTINGS || cur_ep >= CONFIG_USBHOST_MAX_ENDPOINTS || cur_ep >= cur_ep_num) {
                        USB_LOG_ERR("Endpoint num %d overflow\r\n", cur_ep);
                        return -USB_ERR_NOMEM;
                    }
...
int usbh_initialize(uint8_t busid, uintptr_t reg_base, usbh_event_handler_t event_handler)
{
    ...
    extern const struct usbh_class_info camera_usb_classes[2];
