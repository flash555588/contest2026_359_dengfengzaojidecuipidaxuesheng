...
/* intr_adapter_to_nuttx is declared by espressif/esp_irq.h. */
...
#endif
{
  ...
  /* A local IRQ mask alone does not prevent NuttX from preempting a task
   * at a scheduler unlock (for example, when releasing the heap mutex).
   * The recursive spinlock and saved IRQ state are owned by this CPU, so
   * match rspin_lock_irqsave_nopreempt's scheduler protection as well.
   * HAL critical sections must not perform blocking operations.
   */
...
#  endif
  ...
  /* Release CPU-owned state before a pending task can be scheduled. */
