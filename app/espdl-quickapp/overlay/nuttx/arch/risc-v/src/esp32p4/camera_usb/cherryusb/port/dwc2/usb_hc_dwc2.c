...
struct dwc2_chan {
    ...
    /* DWC2 still needs a valid DMA target for a zero-length IN status. */
    uint8_t status_buffer[64] __attribute__((aligned(64)));
};
...
static inline int dwc2_set_mode(struct usbh_bus *bus, uint8_t mode)
{
...
static void dwc2_control_urb_init(struct usbh_bus *bus, uint8_t chidx, struct usbh_urb *urb, struct usb_setup_packet *setup, uint8_t *buffer, uint32_t buflen)
{
    ...
    } else if (chan->ep0_state == DWC2_EP0_STATE_INSTATUS) /* fill in status */
    {
        ...
        dwc2_chan_init(bus,
                       ...
                       1,
                       urb->hport->speed);
        dwc2_chan_transfer(bus, chidx, 0x80, chan->status_buffer, chan->xferlen, chan->num_packets, HC_PID_DATA1);
...
int usb_hc_init(struct usbh_bus *bus)
{
    ...
    extern int camera_usb_phy_ready(void);
    ...
    /* ESP32-P4 HS PHY needs five additional PHY clocks, per its HAL. */
    USB_OTG_GLB->GUSBCFG =
        (USB_OTG_GLB->GUSBCFG & ~USB_OTG_GUSBCFG_TOCAL) | 5U;
    USB_LOG_INFO("P4 UTMI width=%u GUSBCFG=%08lx\r\n",
                 g_dwc2_hcd[bus->hcd.hcd_id].user_params.phy_utmi_width,
                 (unsigned long)USB_OTG_GLB->GUSBCFG);
...
int usbh_submit_urb(struct usbh_urb *urb)
{
    ...
    /* Serialize channel publication and DMA programming with IRQ/kill
     * on the other CPU. Release before a synchronous URB waits. */
    flags = usb_osal_enter_critical_section();
    ...
    return ret;
errout_timeout:
    USB_LOG_ERR("timeout ch=%u ep=%02x state=%u hcchar=%08lx size=%08lx int=%08lx mask=%08lx\r\n",
      chan->chidx, urb->ep->bEndpointAddress, chan->ep0_state,
      (unsigned long)USB_OTG_HC(chan->chidx)->HCCHAR,
      (unsigned long)USB_OTG_HC(chan->chidx)->HCTSIZ,
      (unsigned long)USB_OTG_HC(chan->chidx)->HCINT,
      (unsigned long)USB_OTG_HC(chan->chidx)->HCINTMSK);
    urb->timeout = 0;
    usbh_kill_urb(urb);
    return ret;
}
