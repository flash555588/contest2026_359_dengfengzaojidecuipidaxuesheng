...
static int pwm_stop_timer(struct pwm_lowerhalf_s *dev);
...
static mutex_t g_ledc_lock = NXMUTEX_INITIALIZER;
static uint32_t g_ledc_open_timers;
...
static int pwm_setup(struct pwm_lowerhalf_s *dev)
{
  ...
  g_ledc_open_timers |= 1u << priv->timer;
  nxmutex_unlock(&g_ledc_lock);
  return OK;
}
...
{
  struct esp_ledc_s *priv = (struct esp_ledc_s *)dev;
  int ret = nxmutex_lock(&g_ledc_lock);
  if (ret < 0)
    {
      return ret;
    }
  ...
  if (p_ledc_obj != NULL)
    {
      p_ledc_obj->glb_clk_is_acquired[priv->timer] = false;
    }

  /* All four timers share the HAL and clocks. Closing one must not destroy
   * another timer's live context or switch off its waveform.
   */

  if (p_ledc_obj != NULL && g_ledc_open_timers == 0)
    {
