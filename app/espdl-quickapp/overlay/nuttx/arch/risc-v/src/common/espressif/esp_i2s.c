...
struct esp_i2s_s
{
  ...
  bool tx_dma_armed;              /* TX DMA was started without resetting TX */

  struct esp_transport_s rx;  /* RX transport state */

  bool rx_started;                /* RX channel started */
  bool rx_dma_armed;              /* RX DMA was started without resetting RX */

  bool streaming;                 /* Is I2S peripheral active? */
  bool stopping;                  /* Explicit STOP is retiring the stream */
...
/* ESP-Claw I2S ABI adapters */
/* Preserve legacy result bits; do not cast function pointers. */
#ifdef CONFIG_ESPRESSIF_I2S0_RX
static uint32_t i2s_rxsamplerate_abi(struct i2s_dev_s *dev, uint32_t rate)
{
  return (uint32_t)i2s_rxsamplerate(dev, rate);
}
#endif
#ifdef CONFIG_ESPRESSIF_I2S0_TX
static uint32_t i2s_txsamplerate_abi(struct i2s_dev_s *dev, uint32_t rate)
{
  return (uint32_t)i2s_txsamplerate(dev, rate);
}
#endif
#ifdef CONFIG_ESPRESSIF_I2S0_RX
static uint32_t i2s_rxdatawidth_abi(struct i2s_dev_s *dev, int bits)
{
  return (uint32_t)i2s_rxdatawidth(dev, bits);
}
#endif
#ifdef CONFIG_ESPRESSIF_I2S0_TX
static uint32_t i2s_txdatawidth_abi(struct i2s_dev_s *dev, int bits)
{
  return (uint32_t)i2s_txdatawidth(dev, bits);
}
#endif
static uint32_t i2s_getmclkfrequency_abi(struct i2s_dev_s *dev)
{
  return (uint32_t)i2s_getmclkfrequency(dev);
}
static uint32_t i2s_setmclkfrequency_abi(struct i2s_dev_s *dev, uint32_t frequency)
{
  return (uint32_t)i2s_setmclkfrequency(dev, frequency);
}

static const struct i2s_ops_s g_i2sops =
{
  .i2s_txchannels     = i2s_txchannels,
  .i2s_txsamplerate   = i2s_txsamplerate_abi,
  .i2s_txdatawidth    = i2s_txdatawidth_abi,
  .i2s_send           = i2s_send,

  .i2s_rxchannels     = i2s_rxchannels,
  .i2s_rxsamplerate   = i2s_rxsamplerate_abi,
  .i2s_rxdatawidth    = i2s_rxdatawidth_abi,
  .i2s_receive        = i2s_receive,

  .i2s_ioctl             = i2s_ioctl,
  .i2s_getmclkfrequency  = i2s_getmclkfrequency_abi,
  .i2s_setmclkfrequency  = i2s_setmclkfrequency_abi,
};
...
static struct esp_buffer_s *i2s_buf_allocate(struct esp_i2s_s *priv)
{
  ...
  flags = spin_lock_irqsave_nopreempt(&priv->slock);
  bfcontainer = priv->bf_freelist;
  DEBUGASSERT(bfcontainer);

  /* Reserve the container before allocating. Heap operations can block and
   * must not run while holding the queue spinlock.
   */

  priv->bf_freelist = bfcontainer->flink;
  spin_unlock_irqrestore_nopreempt(&priv->slock, flags);
  ...
  for (i = 0; i < I2S_DMADESC_NUM; i++)
    {
      ...
      if (bfcontainer->dma_link[i] == NULL)
        {
                        ...
                        struct esp_buffer_s *bfcontainer)
{
  ...
  /* This container is no longer queued or owned by DMA. Free its private
   * descriptors before making it available to another submission.
   */
...
/* Return the last descriptor which belongs to one submitted audio buffer.
 * RX may link that descriptor to the next submitted buffer for gapless DMA,
 * so walking until a NULL link is not sufficient once the stream is armed.
 */

static inline lldesc_t *i2s_buffer_lastdesc(
  struct esp_buffer_s *bfcontainer)
{
  unsigned int index = (bfcontainer->nbytes - 1) /
                       I2S_DMA_BUFFER_MAX_SIZE;
  DEBUGASSERT(index < I2S_DMADESC_NUM);
  return bfcontainer->dma_link[index];
}
...
static int IRAM_ATTR i2s_txdma_start(struct esp_i2s_s *priv)
{
  ...
  /* DMA submission owns TX channel lifetime.  Going through the channel
   * helper pairs stop/start bookkeeping and restores the P4 full-duplex
   * clock policy after a prior stream released the device. */

  was_started = priv->tx_started;
  if (!was_started)
    {
      i2s_tx_channel_start(priv);
    }

  /* The first transfer of a stream resets the transmitter and its FIFO so
   * that both start from a known state. Every later transfer keeps I2S and
   * its FIFO running, but resets the now-idle GDMA channel before replacing
   * its descriptor chain. Otherwise a later gdma_start() can leave the old
   * chain live long enough to write back into descriptors already returned
   * to the heap.
   */

  if (!priv->tx_dma_armed && was_started)
    {
      i2s_hal_tx_reset(priv->config->ctx);
    }

  /* Each audio buffer is an independent, NULL-terminated DMA chain. */
  ...
  bfcontainer = (struct esp_buffer_s *)sq_peek(&priv->tx.pend);
  ...
  if (!priv->tx_dma_armed)
    {
      ...
      priv->tx_dma_armed = true;
    }

  priv->tx_started = true;

  sq_remfirst(&priv->tx.pend);
...
static int IRAM_ATTR i2s_rxdma_start(struct esp_i2s_s *priv)
{
  ...
  /* Full-duplex master RX uses TX's BCLK/WS even without playback buffers. */
  ...
  /* The first transfer of a stream resets the receiver and its FIFO so that
   * both start from a known state. Every later transfer keeps I2S and its
   * FIFO running, but resets the now-idle GDMA channel before replacing its
   * descriptor chain. This prevents stale descriptor writeback without
   * discarding a sample at every buffer boundary.
   */

  if (!priv->rx_dma_armed && was_started)
    {
  ...
  bfcontainer = (struct esp_buffer_s *)sq_peek(&priv->rx.pend);
  ...
  if (!priv->rx_dma_armed)
    {
      i2s_hal_rx_start(priv->config->ctx);
      priv->rx_dma_armed = true;
    }
                                     ...
                                     struct esp_buffer_s *bfcontainer)
{
  ...
  if (ret < 0)
    {
      sq_rem((sq_entry_t *)bfcontainer, &priv->tx.pend);
    }
                                     ...
                                     struct esp_buffer_s *bfcontainer)
{
  ...
  bfcontainer->buf = kmm_memalign(alignment,
                                ALIGN_UP(bfcontainer->nbytes, alignment));
...
#if SOC_CACHE_INTERNAL_MEM_VIA_L1CACHE
  esp_cache_msync(bfcontainer->buf,
                  bfcontainer->nbytes,
                  (ESP_CACHE_MSYNC_FLAG_DIR_C2M |
                   ESP_CACHE_MSYNC_FLAG_UNALIGNED));
#endif
  ...
  /* Never edit a DMA-owned descriptor: the engine may already have fetched
   * its NULL next pointer, and a cache writeback can overwrite DMA's updated
   * owner/length fields. Queue software buffers and hand over at EOF instead.
   * rxdma_start preserves the running receiver/FIFO between transfers.
   */
  ...
  if (ret < 0)
    {
      sq_rem((sq_entry_t *)bfcontainer, &priv->rx.pend);
    }
          ...
          if (sq_empty(&priv->tx.act))
            {
              ...
              if (ret < 0)
                {
                  while ((bfcontainer = (struct esp_buffer_s *)
                          sq_remfirst(&priv->tx.pend)) != NULL)
                    {
                      bfcontainer->result = ret;
                      sq_addlast((sq_entry_t *)bfcontainer, &priv->tx.done);
                    }
                }
            }
  ...
  if (!sq_empty(&priv->rx.act))
    {
      ...
      bfdesc = i2s_buffer_lastdesc(bfcontainer);

      if (bfdesc == inlink)
        {
          sq_remfirst(&priv->rx.act);

          /* Detach the completed container from the live DMA chain before
           * the worker walks and copies its descriptors.
           */

          STAILQ_NEXT(bfdesc, qe) = NULL;
  ...
  for (; ; )
    {
      ...
      /* Balance i2s_send's reference, independently of the codec's owner. */
...
static void i2s_rx_worker(void *arg)
{
  ...
  for (; ; )
    {
      ...
      if (bfcontainer->result == OK)
        {
          size_t alignment = cache_hal_get_cache_line_size(
                               CACHE_LL_LEVEL_INT_MEM, CACHE_TYPE_DATA);

          /* RX storage owns whole cache lines. Invalidate it only after
           * DMA has completed; M2C does not accept the UNALIGNED flag.
           */

          if (esp_cache_msync(bfcontainer->buf,
                             ALIGN_UP(bfcontainer->nbytes, alignment),
      ...
      dmadesc = bfcontainer->result == OK ? bfcontainer->dma_link[0] : NULL;

      while (dmadesc != NULL)
        {
          if (dmadesc->length > bfcontainer->apb->nmaxbytes -
      ...
      if (!priv->streaming && !priv->stopping && bfcontainer->result == OK)
        {
      ...
      /* Balance i2s_receive's reference on every returned capture buffer. */
      apb_free(bfcontainer->apb);
  ...
  flags = spin_lock_irqsave_nopreempt(&priv->slock);
  if (!priv->streaming &&
      (!priv->config->tx_en || priv->config->role == I2S_ROLE_MASTER) &&
      (sq_empty(&priv->rx.act) && sq_empty(&priv->rx.pend)))
    {
      i2s_rx_channel_stop(priv);
  ...
  if (priv->config->tx_en)
    {
      ...
      /* RX is a slave of TX's BCLK/WS in full-duplex master mode. The
       * hardware default stops TX when its FIFO is empty, which otherwise
       * prevents microphone capture whenever no playback DMA is queued.
       */

      if (priv->config->rx_en && priv->config->role == I2S_ROLE_MASTER)
        {
          priv->config->ctx->dev->tx_conf.tx_stop_en = 0;
        }
#endif
...
{
  struct esp_i2s_s *priv = (struct esp_i2s_s *)arg;
  struct esp_buffer_s *buffer;
  lldesc_t *desc;
  lldesc_t *cur = NULL;
  irqstate_t flags;

  flags = spin_lock_irqsave_nopreempt(&priv->slock);
  buffer = (struct esp_buffer_s *)sq_peek(&priv->tx.act);
  if (event_data->flags.normal_eof)
    {
      cur = (lldesc_t *)(event_data->tx_eof_desc_addr);

      /* STOP can retire the descriptor before this ISR obtains the lock.
       * Validate it against the active chain before dereferencing it.
       */

      desc = buffer == NULL ? NULL : buffer->dma_link[0];
      while (desc != NULL && desc != cur)
        {
      ...
      if (desc != NULL && STAILQ_NEXT(cur, qe) == NULL)
        {
 ...
 *   transmit DMA descriptor error occurs and returns owned buffers with
 *   an I/O error so a later stream can recover.
 *
 * Input Parameters:
 *   dma_chan    - GDMA channel handle
 *   event_data  - Pointer to GDMA event data structure
                                   ...
                                   void *arg)
{
  ...
  /* A malformed/stale descriptor must fail the stream, not the system.
   * Stop and reset GDMA before the normal worker frees any descriptor memory;
   * descriptor-error interrupts do not stop the channel automatically.
   */
  ...
  flags = spin_lock_irqsave_nopreempt(&priv->slock);
  priv->tx_dma_armed = false;
  while ((buffer = (struct esp_buffer_s *)sq_remfirst(&priv->tx.act)) != NULL)
    {
      buffer->result = -EIO;
      sq_addlast((sq_entry_t *)buffer, &priv->tx.done);
    }

  while ((buffer = (struct esp_buffer_s *)sq_remfirst(&priv->tx.pend)) != NULL)
    {
      buffer->result = -EIO;
      sq_addlast((sq_entry_t *)buffer, &priv->tx.done);
    }
 ...
 *   receive DMA descriptor error occurs and returns owned buffers with
 *   an I/O error so a later stream can recover.
 *
 * Input Parameters:
 *   dma_chan    - GDMA channel handle
                                   ...
                                   void *arg)
{
  ...
  /* The DMA engine may still own the failing descriptor until explicitly
   * stopped. Do this before handing its storage to the worker for release.
   */
  ...
  flags = spin_lock_irqsave_nopreempt(&priv->slock);
  priv->rx_dma_armed = false;
  while ((buffer = (struct esp_buffer_s *)sq_remfirst(&priv->rx.act)) != NULL)
    {
      buffer->result = -EIO;
      sq_addlast((sq_entry_t *)buffer, &priv->rx.done);
    }

  while ((buffer = (struct esp_buffer_s *)sq_remfirst(&priv->rx.pend)) != NULL)
    {
      buffer->result = -EIO;
      sq_addlast((sq_entry_t *)buffer, &priv->rx.done);
    }
...
#endif
  flags = spin_lock_irqsave_nopreempt(&priv->slock);
  buffer = (struct esp_buffer_s *)sq_peek(&priv->rx.act);
  if (event_data->flags.normal_eof && buffer != NULL)
    {
      cur = (lldesc_t *)(event_data->rx_eof_desc_addr);

      desc = buffer->dma_link[0];
      while (desc != NULL && desc != cur &&
             desc != i2s_buffer_lastdesc(buffer))
        {
          desc = STAILQ_NEXT(desc, qe);
        }
      ...
      if (cur == i2s_buffer_lastdesc(buffer))
        {
      ...
      case AUDIOIOC_START:
        {
          ...
          printf("[audio-state] i2s start tx=%d/%d rx=%d/%d rate=%" PRIu32
                 " width=%u\n", priv->tx_started, priv->tx_dma_armed,
                 priv->rx_started, priv->rx_dma_armed, priv->rate,
                 priv->data_width);
      ...
      case AUDIOIOC_STOP:
        {
          ...
          printf("[audio-state] i2s stop tx=%d/%d rx=%d/%d stream=%d\n",
                 priv->tx_started, priv->tx_dma_armed, priv->rx_started,
                 priv->rx_dma_armed, priv->streaming);
          ...
          /* Stop DMA before returning buffers. Waiting for natural EOF can
           * never complete when the codec or its clock has already stopped.
           */

          ret = i2s_rx_channel_stop(priv);
          if (ret == OK)
            {
              ret = i2s_tx_channel_stop(priv);
            }

          if (ret == OK)
            {
              while ((buffer = (struct esp_buffer_s *)sq_remfirst(&priv->rx.act)))
                {
                  buffer->result = -ECANCELED;
                  sq_addlast((sq_entry_t *)buffer, &priv->rx.done);
                }

              while ((buffer = (struct esp_buffer_s *)sq_remfirst(&priv->rx.pend)))
                {
                  buffer->result = -ECANCELED;
                  sq_addlast((sq_entry_t *)buffer, &priv->rx.done);
                }

              while ((buffer = (struct esp_buffer_s *)sq_remfirst(&priv->tx.act)))
                {
                  buffer->result = -ECANCELED;
                  sq_addlast((sq_entry_t *)buffer, &priv->tx.done);
                }

              while ((buffer = (struct esp_buffer_s *)sq_remfirst(&priv->tx.pend)))
                {
                  buffer->result = -ECANCELED;
                  sq_addlast((sq_entry_t *)buffer, &priv->tx.done);
                }
          ...
          /* A work item becomes "available" before its callback returns, so
           * work_available() is not a completion barrier.  Synchronize with
           * any queued/running drains, then drain the buffers retired above
           * in this thread.  No DMA interrupt can add more work after both
           * channels have stopped and their active queues have been removed.
           * This guarantees that every I2S callback has released its APB
           * reference before AUDIOIOC_STOP returns to the audio upper half.
           */

          work_cancel_sync(HPWORK, &priv->rx.work);
          work_cancel_sync(HPWORK, &priv->tx.work);
          i2s_rx_worker(priv);
          i2s_tx_worker(priv);
