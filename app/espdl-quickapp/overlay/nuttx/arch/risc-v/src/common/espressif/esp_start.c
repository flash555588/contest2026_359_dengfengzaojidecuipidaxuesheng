      ...
      if (ram_segments == image_header.segment_count && !padding_checksum)
        {
          ...
          if (image_header.hash_appended)
            {
              /* ROM checks the digest of the visible RAM image. */
...
{
  ets_printf("V1: soc startup begin\n");
  sys_startup_fn();
  ets_printf("V1: soc startup end\n");
}
...
void __esp_start(void)
{
  ...
  /* Enable scalar floating point before any hard-float C code executes.
   * Inline here: flash mappings are not established during Simple Boot. */
  __asm__ __volatile__("li t0, 0x2000\ncsrs mstatus, t0\nfscsr zero"
                       ::: "t0", "memory");
#endif
  esp_err_t ret;
...
#ifdef CONFIG_ESP32P4_SELECTS_REV_LESS_V3
  v1_progress("V1: MSPI pins reserved\n");
#endif
...
#  ifdef CONFIG_ESP32P4_SELECTS_REV_LESS_V3
  v1_progress("V1: brownout init begin\n");
#  endif
  esp_brownout_init();
#  ifdef CONFIG_ESP32P4_SELECTS_REV_LESS_V3
  v1_progress("V1: brownout init ready\n");
#  endif
#endif
...
#ifdef CONFIG_ESP32P4_SELECTS_REV_LESS_V3
  v1_progress("V1: UART low setup ready\n");
#endif
...
#ifdef CONFIG_ESP32P4_SELECTS_REV_LESS_V3
  v1_progress("V1: chip revision check ready\n");
#endif
