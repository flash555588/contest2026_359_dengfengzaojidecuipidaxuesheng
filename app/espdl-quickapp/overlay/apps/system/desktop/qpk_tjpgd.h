...
#if JD_FASTDECODE >= 1
typedef int16_t qpk_jd_yuv_t;
#else
typedef uint8_t qpk_jd_yuv_t;
#endif
...
struct JDEC {
    ...
    qpk_jd_yuv_t * mcubuf;          /* Working buffer for the MCU */
...
JRESULT qpk_jd_prepare(JDEC * jd, size_t (*infunc)(JDEC *, uint8_t *, size_t), void * pool, size_t sz_pool, void * dev);

JRESULT qpk_jd_decomp(JDEC * jd, int (*outfunc)(JDEC *, void *, JRECT *), uint8_t scale);

JRESULT qpk_jd_mcu_load(JDEC * jd);

JRESULT qpk_jd_mcu_output(JDEC * jd, int (*outfunc)(JDEC *, void *, JRECT *), unsigned int x, unsigned int y);

JRESULT qpk_jd_restart(JDEC * jd, uint16_t rstn);
