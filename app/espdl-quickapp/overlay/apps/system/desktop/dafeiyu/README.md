...
无需打开桌宠控制面板：在桌面上长按大肥鱼开始语音识别，按住说话，松手后自动把
识别结果发送给 ESPClaw，流式回复和最终回复直接显示在桌宠气泡中。录音期间轻微移动
不会误触发拖拽，长按也不会继续触发原有单击或双击动作。首次使用仍需在系统服务配置
中填写语音识别凭据和 AI 服务配置；未配置、服务忙碌或网络失败时会直接冒泡提示。
