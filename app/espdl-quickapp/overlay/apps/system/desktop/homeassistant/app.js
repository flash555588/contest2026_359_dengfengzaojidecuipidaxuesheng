...
(function () {
  ...
  function parseUrl(value) {
    ...
    const match = /^http:\/\/([a-zA-Z0-9._-]+)(?::([0-9]{1,5}))?\/?$/.exec(value.trim());
    if (!match) return '';
    const host = match[1].toLowerCase();
    if (match[2] && (Number(match[2]) < 1 || Number(match[2]) > 65535)) return '';
    return 'http://' + host + (match[2] ? ':' + Number(match[2]) : '');
  }
  ...
  function credentialsReady() {
    ...
    else if (!parseUrl(url)) message = '需要有效的 HTTP 地址';
    ...
    prompt.input({ title: 'Home Assistant 地址', value: url, maxLength: 120 }, value => {
      ...
      if (!next) {
        message = /^https:/i.test(String(value)) ?
          '当前 HA 接口不支持 HTTPS，不会降级连接' : '仅支持 HTTP 地址，不允许路径或账号';
      } else if (next !== url) {
        invalidate();
        token = '';
        httpAllowed = false;
