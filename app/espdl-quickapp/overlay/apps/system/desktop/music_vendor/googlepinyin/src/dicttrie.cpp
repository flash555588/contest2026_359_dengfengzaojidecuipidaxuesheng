...
bool DictTrie::save_dict(FILE *fp) {
  ...
  if (gp_write_sizes(&lma_node_num_le0_, 1, fp) != 1)
    return false;

  if (gp_write_sizes(&lma_node_num_ge1_, 1, fp) != 1)
    return false;

  if (gp_write_sizes(&lma_idx_buf_len_, 1, fp) != 1)
    return false;

  if (gp_write_sizes(&top_lmas_num_, 1, fp) != 1)
    return false;
...
bool DictTrie::load_dict(FILE *fp) {
  ...
  if (gp_read_sizes(&lma_node_num_le0_, 1, fp) != 1)
    return false;

  if (gp_read_sizes(&lma_node_num_ge1_, 1, fp) != 1)
    return false;

  if (gp_read_sizes(&lma_idx_buf_len_, 1, fp) != 1)
    return false;

  if (gp_read_sizes(&top_lmas_num_, 1, fp) != 1 ||
...
bool DictTrie::load_dict(const char *filename, LemmaIdType start_id,
  ...
  FILE *fp = !strcmp(filename, "@builtin") ? glass_pinyin_dictionary_open() : fopen(filename, "rb");
  if (NULL == fp)
    return false;
