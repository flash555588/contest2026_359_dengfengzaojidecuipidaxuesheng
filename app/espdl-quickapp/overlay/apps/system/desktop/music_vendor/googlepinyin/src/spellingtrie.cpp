...
bool SpellingTrie::save_spl_trie(FILE *fp) {
  ...
  if (gp_write_sizes(&spelling_size_, 1, fp) != 1)
    return false;

  if (gp_write_sizes(&spelling_num_, 1, fp) != 1)
    return false;
...
bool SpellingTrie::load_spl_trie(FILE *fp) {
  ...
  if (gp_read_sizes(&spelling_size_, 1, fp) != 1)
    return false;

  if (gp_read_sizes(&spelling_num_, 1, fp) != 1)
    return false;
