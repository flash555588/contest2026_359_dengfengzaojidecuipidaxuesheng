...
bool DictList::save_list(FILE *fp) {
  ...
  if (gp_write_sizes(&scis_num_, 1, fp) != 1)
    return false;

  if (gp_write_sizes(start_pos_, kMaxLemmaSize + 1, fp) !=
      kMaxLemmaSize + 1)
    return false;

  if (gp_write_sizes(start_id_, kMaxLemmaSize + 1, fp) !=
      kMaxLemmaSize + 1)
    return false;
...
bool DictList::load_list(FILE *fp) {
  ...
  if (gp_read_sizes(&scis_num_, 1, fp) != 1)
    return false;

  if (gp_read_sizes(start_pos_, kMaxLemmaSize + 1, fp) !=
      kMaxLemmaSize + 1)
    return false;

  if (gp_read_sizes(start_id_, kMaxLemmaSize + 1, fp) !=
      kMaxLemmaSize + 1)
    return false;
