...
static void block_idct(
    ...
    qpk_jd_yuv_t * dst  /* Pointer to the destination to store the block as byte array */
)
{
    ...
    for(blk = 0; blk < nby + 2; blk++) {    /* Get nby Y blocks and two C blocks */
        ...
        if(cmp && jd->ncomp != 3) {         /* Clear C blocks if not exist (monochrome image) */
            ...
            if(JD_FORMAT != 2 || !cmp) {    /* C components may not be processed if in grayscale output */
                if(z == 1) {    /* A constant block needs no IDCT. */
                    d = (qpk_jd_yuv_t)((*tmp / 256) + 128);
                    if(JD_FASTDECODE >= 1) {
                        for(i = 0; i < 64; bp[i++] = d) ;
                    }
                    else {
...
)
{
    ...
    {   /* Build full MCU pixels before reducing them. */
    ...
    /* The LVGL decoder this was derived from omitted output scaling. Merely
     * shrinking the rectangle copies unrelated MCU rows into the image.
     * Average each complete 2x2, 4x4 or 8x8 block in place before cropping. */
    if(JD_USE_SCALE && jd->scale) {
        unsigned int step = 1U << jd->scale;
        unsigned int channels = JD_FORMAT == 2 ? 1 : 3;
        uint8_t *src = (uint8_t *)jd->workbuf;
        uint8_t *dst = src;
        for(unsigned int sy = 0; sy < my; sy += step) {
            for(unsigned int sx = 0; sx < mx; sx += step) {
                for(unsigned int channel = 0; channel < channels; channel++) {
                    unsigned int sum = 0;
                    for(unsigned int dy = 0; dy < step; dy++) {
                        for(unsigned int dx = 0; dx < step; dx++) {
                            sum += src[((sy + dy) * mx + sx + dx) * channels + channel];
                        }
                    }
                    *dst++ = sum >> (jd->scale * 2);
                }
    ...
    for(;;) {               /* Parse JPEG segments */
        ...
        switch(marker & 0xFF) {
            ...
            case 0xDA:  /* SOS - Start of Scan */
                ...
                jd->mcubuf = alloc_pool(jd, (n + 2) * 64 * sizeof(qpk_jd_yuv_t));   /* Allocate MCU working buffer */
    ...
    for(y = 0; y < jd->height; y += my) {       /* Vertical loop of MCUs */
        for(x = 0; x < jd->width; x += mx) {    /* Horizontal loop of MCUs */
            if(jd->nrst && rst++ == jd->nrst) {     /* Process restart interval if enabled */
                rc = qpk_jd_restart(jd, rsc++);
                if(rc != JDR_OK) return rc;
                rst = 1;
            }
            rc = qpk_jd_mcu_load(jd);                  /* Load an MCU (decompress huffman coded stream, dequantize and apply IDCT) */
            if(rc != JDR_OK) return rc;
            rc = qpk_jd_mcu_output(jd, outfunc, x, y); /* Output the MCU (YCbCr to RGB, scaling and output) */
            if(rc != JDR_OK) return rc;
        }
    }
