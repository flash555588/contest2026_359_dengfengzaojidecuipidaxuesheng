...
static void command_event(struct ble_npl_event *event)
{
  ...
  if (command == 1)
    ...
    ret = ble_gap_conn_cancel();
  pthread_mutex_lock(&g_lock);
  if (ret)
    {
      g_state.error = ret;
      if (command == 1) g_state.scanning = false;
      if (command == 2) { g_state.connecting = false; g_cancel_connect = false; }
    }
  if (command == 3 && !ret) g_state.scanning = false;
  bool cancel = g_cancel_scan && g_state.scanning;
  bool cancel_connect = g_cancel_connect && g_state.connecting;
  g_cancel_scan = false;
  if (g_command == command) g_command = 0;
  pthread_mutex_unlock(&g_lock);
  if (cancel)
    {
      ret = ble_gap_disc_cancel();
  ...
  if (cancel_connect) {
    ret = ble_gap_conn_cancel();
    pthread_mutex_lock(&g_lock);
    if (ret && ret != BLE_HS_EALREADY) g_state.error = ret;
    pthread_mutex_unlock(&g_lock);
  }
}
...
static void reset(int reason)
{
  ...
  g_cancel_scan = g_cancel_connect = false;
  g_command = 0;
...
{
  pthread_mutex_lock(&g_lock);
  if (g_command == 1 || g_command == 2)
    {
      if (g_command == 1) g_cancel_scan = true;
      else g_cancel_connect = true;
      pthread_mutex_unlock(&g_lock);
      return 0;
    }
  bool scanning = g_state.scanning;
  bool connecting = g_state.connecting;
  if (connecting) g_cancel_connect = true;
  pthread_mutex_unlock(&g_lock);
  return scanning ? submit(3, 0) : connecting ? submit(5, 0) : 0;
}
