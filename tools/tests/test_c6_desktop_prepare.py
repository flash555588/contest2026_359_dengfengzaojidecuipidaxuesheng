...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert 'g_wifi_event_lock' in (out / 'esp_hosted_rpc.c').read_text()
        assert 'esp_hosted_rpc_wifi_scan_results' in (out / 'esp_hosted_rpc.c').read_text()
        assert 'volatile bool' not in (out / 'c6net.c').read_text()
        ...
        invalid.mkdir()
        for name in ('esp_hosted.c', 'esp_hosted_rpc.c', 'c6net.c', 'c6net.h', 'rpc_mailbox.h'):
