...
/* P4 HAL uses esprv_int_*; v0/v1 ROM ld still PROVIDEs those from
 * esprv_intc_int_*. Keep both names as real symbols.
 */
 ...
 *
 * Description:
 *   Map a NuttX IRQ number onto the compact vectortable index used when
 *   CONFIG_ARCH_MINIMAL_VECTORTABLE is enabled. Peripheral IRQs share the
 *   demux slot; exceptions keep their native numbers.
 *
 ****************************************************************************/
...
#ifdef CONFIG_ARCH_CHIP_ESP32P4
int esp_setup_irq_with_flags_intrstatus(int source, int flags,
                                        uint32_t intrstatusreg,
                                        uint32_t intrstatusmask,
                                        xcpt_t handler, void *arg)
{
  irq_priority_t priority = ESP_IRQ_PRIORITY_DEFAULT;
  int type = (flags & (1 << 9)) ? ESP_IRQ_TRIGGER_EDGE :
             ESP_IRQ_TRIGGER_LEVEL;
  int cpuint;
  int irq;

  UNUSED(intrstatusreg);
  UNUSED(intrstatusmask);
...
int IRAM_ATTR esp_get_cpuint(int cpu, int irq)
{
  ...
  for (i = 0; i < ESP_NCPUINTS; i++)
    {
      if (CPUINT_ISASSIGNED(g_cpuint_map[i]) &&
          CPUINT_GETIRQ(g_cpuint_map[i]) == irq)
        {
          return i;
        }
    }
