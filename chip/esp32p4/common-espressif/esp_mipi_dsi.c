...
/* Keep PSRAM cache maintenance below the ESP32-P4 IDF default chunk size.
...
#  define CONFIG_ESPRESSIF_MIPI_DSI_LANE_BITRATE_MBPS 900
#endif
...
static int esp_mipi_dsi_cache_writeback(FAR void *addr, size_t len)
{
  FAR uint8_t *cursor = (FAR uint8_t *)addr;
  esp_err_t err;
  size_t total = len;
  size_t chunks = 0;
  size_t chunk;
  ...
  while (len > 0)
    {
      chunk = len > ESP_MIPI_DSI_CACHE_CHUNK ?
              ESP_MIPI_DSI_CACHE_CHUNK : len;
      err = esp_cache_msync(cursor, chunk,
  ...
  vinfo("esp_mipi_dsi: cache writeback %zu bytes in %zu chunks\n",
        total, chunks);
  return OK;
}
                                  ...
                                  uint8_t bpp)
{
  ...
  /* ESP32-P4 block_ts is 22 bits, so one LLI carries the complete frame.
   * ESP-IDF uses a singly-linked one-item list and re-arms it from the
   * full-transfer interrupt.  Do not split at 4095: doing so corrupts the
   * list contract and makes the bridge see incomplete frames.
   */
  ...
  ret = esp_mipi_dsi_cache_writeback(fb, fb_size);
  if (ret < 0)
    {
      nxmutex_unlock(&priv->lock);
      return ret;
...
int esp_mipi_dsi_flush_framebuffer(FAR void *addr, size_t len)
{
  ...
  return esp_mipi_dsi_cache_writeback(addr, len);
}

/****************************************************************************
 * Name: esp_mipi_dsi_set_test_pattern
 ****************************************************************************/

int esp_mipi_dsi_set_test_pattern(bool enable)
{
  FAR struct esp_mipi_dsi_priv_s *priv = &g_esp_mipi_dsi;
  int ret;
  ...
  if (!priv->initialized || !priv->dpi_configured || priv->video_running)
    {
      nxmutex_unlock(&priv->lock);
      return -EAGAIN;
    }
  ...
  mipi_dsi_host_ll_dpi_set_pattern_type(
    priv->hal.host,
    enable ? MIPI_DSI_PATTERN_BAR_VERTICAL : MIPI_DSI_PATTERN_NONE);
  priv->test_pattern_enabled = enable;
  nxmutex_unlock(&priv->lock);
  return OK;
}

/****************************************************************************
 * Name: esp_mipi_dsi_get_diagnostics
 *
 * Description:
 *   Snapshot the video DMA and bridge status for board bring-up.
 ****************************************************************************/

int esp_mipi_dsi_get_diagnostics(FAR uint32_t *dma_frames,
                                 FAR uint32_t *dma_status,
                                 FAR uint32_t *bridge_status,
                                 FAR uint32_t *lane_mbps_x100,
                                 FAR uint32_t *dpi_expect_mhz_x100,
                                 FAR uint32_t *dpi_real_mhz_x100,
                                 FAR uint32_t *host_int_st0,
                                 FAR uint32_t *host_int_st1,
                                 FAR uint32_t *host_vid_pkt_status,
                                 FAR uint32_t *host_phy_status,
                                 FAR uint32_t *host_color_coding)
{
  FAR struct esp_mipi_dsi_priv_s *priv = &g_esp_mipi_dsi;

  if (dma_frames == NULL || dma_status == NULL || bridge_status == NULL ||
      lane_mbps_x100 == NULL || dpi_expect_mhz_x100 == NULL ||
      dpi_real_mhz_x100 == NULL || host_int_st0 == NULL ||
      host_int_st1 == NULL || host_vid_pkt_status == NULL ||
      host_phy_status == NULL || host_color_coding == NULL ||
      !priv->initialized)
    {
      return -EINVAL;
    }

  *dma_frames = priv->dma_tfr_count;
  *dma_status = priv->dw_hal.dev != NULL ? priv->dma_last_status : 0;
  *bridge_status = mipi_dsi_brg_ll_get_interrupt_status(priv->hal.bridge);
  *lane_mbps_x100 = (uint32_t)lroundf(priv->hal.lane_bit_rate_mbps * 100.0f);
  *dpi_expect_mhz_x100 =
    (uint32_t)lroundf(priv->hal.expect_dpi_clock_freq_mhz * 100.0f);
  *dpi_real_mhz_x100 =
    (uint32_t)lroundf(priv->hal.real_dpi_clock_freq_mhz * 100.0f);
  *host_int_st0 = priv->hal.host->int_st0.val;
  *host_int_st1 = priv->hal.host->int_st1.val;
  *host_vid_pkt_status = priv->hal.host->vid_pkt_status.val;
  *host_phy_status = priv->hal.host->phy_status.val;
  *host_color_coding = priv->hal.host->dpi_color_coding_act.val;
  return OK;
}

/****************************************************************************
 * Name: esp_mipi_dsi_get_timing_diagnostics
 *
 * Description:
 *   Read back the bridge timing and the programmed DSI host registers.
 ****************************************************************************/
      ...
      FAR struct esp_mipi_dsi_timing_diagnostics_s *diag)
{
  ...
  FAR dsi_host_dev_t *host;

  if (diag == NULL || !priv->initialized)
    {
      return -EINVAL;
    }
  ...
  diag->bridge_raw_words = bridge->raw_num_cfg.raw_num_total;
  diag->bridge_fifo_status = bridge->fifo_flow_status.val;
  diag->bridge_pixel_type = bridge->pixel_type.val;
  diag->bridge_htotal = bridge->dpi_h_cfg0.htotal;
  diag->bridge_hactive = bridge->dpi_h_cfg0.hdisp;
  diag->bridge_hsync = bridge->dpi_h_cfg1.hsync;
  diag->bridge_hback_porch = bridge->dpi_h_cfg1.hbank;
  diag->bridge_vtotal = bridge->dpi_v_cfg0.vtotal;
  diag->bridge_vactive = bridge->dpi_v_cfg0.vdisp;
  diag->bridge_vsync = bridge->dpi_v_cfg1.vsync;
  diag->bridge_vback_porch = bridge->dpi_v_cfg1.vbank;

  diag->host_packet_pixels = host->vid_pkt_size.vid_pkt_size;
  diag->host_hsync_time = host->vid_hsa_time.vid_hsa_time;
  diag->host_hback_porch_time = host->vid_hbp_time.vid_hbp_time;
  diag->host_hline_time = host->vid_hline_time.vid_hline_time;
  diag->host_vsync_lines = host->vid_vsa_lines.vsa_lines;
