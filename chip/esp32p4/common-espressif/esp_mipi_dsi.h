...
/* Read-back of the live bridge and programmed DSI host video timing. */

struct esp_mipi_dsi_timing_diagnostics_s
{
  uint32_t bridge_raw_words;
  uint32_t bridge_fifo_status;
  uint32_t bridge_pixel_type;
  uint32_t bridge_htotal;
  uint32_t bridge_hactive;
  uint32_t bridge_hsync;
  uint32_t bridge_hback_porch;
  uint32_t bridge_vtotal;
  uint32_t bridge_vactive;
  uint32_t bridge_vsync;
  uint32_t bridge_vback_porch;
  uint32_t host_packet_pixels;
  uint32_t host_hsync_time;
  uint32_t host_hback_porch_time;
  uint32_t host_hline_time;
  uint32_t host_vsync_lines;
  uint32_t host_vback_porch_lines;
  uint32_t host_vfront_porch_lines;
 ...
 *
 * Description:
 *   Enable or disable the DSI Host video pattern generator.  The generated
 *   color bars bypass the framebuffer, PSRAM and DW-GDMA path and are useful
 *   for separating link/timing faults from pixel-feed faults.
 ****************************************************************************/
