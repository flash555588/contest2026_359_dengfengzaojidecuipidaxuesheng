...
{
  static spinlock_t module_lock = SP_UNLOCKED;
  int uart_num = priv->id;
  bool newly_enabled = false;
  irqstate_t flags;

  /* esp_lowsetup() runs before nx_start().  A scheduler-backed mutex cannot
   * be acquired safely here when SMP is enabled, so protect the one-time
   * UART clock transition with the UART's IRQ-safe spinlock instead.
   */
