...
#if defined(CONFIG_ARCH_CHIP_ESP32P4) && !defined(CONFIG_ESPRESSIF_UART0)
  /* Retire ROM UART0 routing before the allocator reuses its CPU line. */
  for (j = 0; j < ESP_NCPUS; j++)
    {
      esp_rom_route_intr_matrix(j, ETS_UART0_INTR_SOURCE, 6);
    }
#endif
                                        ...
                                        void *arg)
{
  ...
  /* Allocation enables the source; publish its map before restoring IRQs. */
