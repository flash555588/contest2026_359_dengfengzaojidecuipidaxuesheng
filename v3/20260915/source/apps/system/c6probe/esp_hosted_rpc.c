...
static struct c6_rpc_mailbox g_rpc = C6_RPC_MAILBOX_INITIALIZER;
                      ...
                      uint8_t pkt_type)
{
  ...
  if (msg->msg_type == RPC_TYPE__Resp &&
...
int esp_hosted_rpc_set_wifi_event_cb(esp_hosted_wifi_event_cb_t cb,
                                     FAR void *arg)
{
  int ret = pthread_mutex_lock(&g_wifi_event_lock);
  if (ret != 0) return -ret;
  g_wifi_event_cb = cb;
                                    ...
                                    size_t capacity, size_t *count)
{
  ...
  *count = 0;
  ...
  if (resp->payload_case != RPC__PAYLOAD_RESP_WIFI_SCAN_GET_AP_NUM ||
      resp->resp_wifi_scan_get_ap_num == NULL)
    {
      rpc__free_unpacked(resp, NULL);
      return -EIO;
    }
  ...
  if (resp->payload_case != RPC__PAYLOAD_RESP_WIFI_SCAN_GET_AP_RECORDS ||
      resp->resp_wifi_scan_get_ap_records == NULL)
    {
      rpc__free_unpacked(resp, NULL);
      return -EIO;
    }

  RpcRespWifiScanGetApRecords *result = resp->resp_wifi_scan_get_ap_records;
