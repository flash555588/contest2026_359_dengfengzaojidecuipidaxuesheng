...
int usb_osal_sem_take(usb_osal_sem_t sem, uint32_t timeout)
{
    ...
    if (timeout == 0xffffffff) {
        ret = nxsem_wait_uninterruptible(__sem);
    } else {
        ret = nxsem_tickwait_uninterruptible(__sem, MSEC2TICK(timeout));
    }
...
int usb_osal_mq_send(usb_osal_mq_t mq, uintptr_t addr)
{
    ...
    ret = file_mq_ticksend(&mq_adpt->mq, (const char *)&addr, mq_adpt->msgsize, 0, 0);
    if (ret < 0) {
        return -USB_ERR_INVAL;
    } else {
        return 0;
...
int usb_osal_mq_recv(usb_osal_mq_t mq, uintptr_t *addr, uint32_t timeout)
{
    ...
    if (timeout == 0xffffffff)
        ...
        if (timeout) {
            msec2spec(&__timeout, MSEC2TICK(timeout));
